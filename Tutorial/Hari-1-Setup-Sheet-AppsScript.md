...
- [ ] Google Sheet dengan 2 tab siap (`Pengunjung`, `Log_Kunjungan`).
- [ ] Apps Script `doPost` ditulis dan tersimpan.
- [ ] Web App sudah di-deploy, URL-nya tercatat.
- [ ] Tes dengan UID dummy berhasil, termasuk verifikasi baris baru benar-benar muncul di tab `Log_Kunjungan`.
...
6. Isi baris header di tab `Log_Kunjungan`:
   ```
   uid_kartu | waktu_tap
   ```

## Langkah 2 — Isi data uji di tab Pengunjung
...
## Langkah 6 — Tes dengan PowerShell
...
Harus membalas `status: OK`, `nama: Budi Testing`. Coba juga dengan UID yang tidak terdaftar (misal `UNKNOWN123`) — harus membalas `REJECTED`.

## Langkah 7 — Verifikasi tercatat di Sheet

Buka tab `Log_Kunjungan` secara langsung di Sheet, pastikan baris baru dengan `uid_kartu` `DEADBEEF` dan timestamp muncul di sana. Balasan JSON `OK` cuma membuktikan kodenya jalan tanpa error, langkah ini yang memastikan datanya benar-benar tersimpan.

Kalau baris itu muncul, Hari 1 selesai — lanjut ke Hari 2 (Google Form pendaftaran).
